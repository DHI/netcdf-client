...
{
    public class ConvertNcToDfs3: iCommand
    {
        ...
        {
            return "Converts a structured grid netcdf file (CF standard) to a dfs3 file";
        }
        ...
        private void _getMinAndInterval(ucar.nc2.Variable ncDim, string minMaxStr, out double interval, out double minVal, out List<double> interpolatedVal, out List<double> originalVal, bool useDefinedDZ)
        {
            ...
            for (int dCount = minCount; dCount <= maxCount; dCount++)
            {
                ...
                if (minVal >= data) minVal = Math.Abs(data);
                if (maxVal <= data) maxVal = Math.Abs(data);

                if (!stepExists)
                {
                    if (dCount > 0)
                    {
                        prevVal = Math.Abs(dataArr.getDouble(dIndex.set(dCount - 1)));
                        interval = Math.Abs(data - prevVal);
